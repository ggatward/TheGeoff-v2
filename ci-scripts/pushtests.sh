...
if [ $(sed -e 's/^.*release //' -e 's/\..*$//' /etc/redhat-release) -ge 7 ]; then
  ...
  setsid ssh -q ${ssh_opts} root@$TEST_VM 'true'
  setsid ssh-copy-id root@$TEST_VM
fi

echo "Installing bats and rsync on test server $TEST_VM"
if ssh -q ${ssh_opts} root@$TEST_VM "yum install -y bats rsync"; then
  echo "copying tests to test server $TEST_VM"
  rsync --delete -va -e "ssh -q ${ssh_opts}" tests/bats_tests root@$TEST_VM:
else
...
if ssh -q ${ssh_opts} root@$TEST_VM "yum install -y pytest python-psutil"; then
  rsync --delete -va -e "ssh -q ${ssh_opts}" tests/unit_tests root@$TEST_VM:
else
...
HOST_GROUP=$(ssh -q ${ssh_opts} root@$TEST_VM "grep Group /etc/soe-release" | cut -f1 -d/ | awk '{print $3}')
RHELVER=$(ssh -q ${ssh_opts} root@$TEST_VM "grep Built /etc/soe-release" | awk '{ print $11 }' | cut -f1 -d.)

# execute the tests in parallel on all test servers
mkdir -p test_results
echo "Starting TAPS tests on test server $TEST_VM"
if [ -z "${HOST_GROUP}" ]; then
  ssh -q ${ssh_opts} root@$TEST_VM \
...
rsync -va -e "ssh -q ${ssh_opts}" root@$TEST_VM:unit_tests/*.xml test_results/
